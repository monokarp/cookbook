...
export function PrepackDetails({ navigation, route }: Props) {
    ...
    const validIngredients = [...products, ...prepacks.filter(p => !containsAsNestedIngredient(p, prepack))];

    const addIngredient = (value: ProductIngredient) =>
        setPrepack(
            new Prepack({
                ...prepack,
    ...
    const setIngredient = (value: Position, index: number) =>
        setPrepack(
            new Prepack({
                ...prepack,
    ...
    const onSubmit = async (data: { name: string; finalWeight: string; description: string }) => {
        ...
        await Promise.all([prepacksRepo.All().then(setPrepacks), recipeRepo.All().then(setRecipes)]);
                        ...
                        <View style={{ flexDirection: "row" }}>
                            <View style={{ flex: 4 }}>
                                ...
                                {form.formState.errors.name && (
                                    <Text testID={TestIds.PrepackDetails.NameInputError} style={styles.validationErrorLabel}>
                                ...
                                <Controller
                                    rules={{
                                        validate: value => {
                                            ...
                                            return prepack.ingredients.length ? isValidWeight : isValidWeight || value === "";
                                        },
                                    }}
                                    render={({ field: { onChange, onBlur, value } }) => (
                                        <TextInput
                                ...
                                {form.formState.errors.finalWeight && (
                                    <Text testID={TestIds.PrepackDetails.WeightInputError} style={styles.validationErrorLabel}>
                                ...
                                <Controller
                                    render={({ field: { onChange, value } }) => <PrepackDescription value={value} onChange={onChange} />}
                                    name="description"
                                />
