...
function _IngredientSelect({
    ...
    const {
        ...
        handleSubmit,
        formState: { errors },
    } = useForm<IngredientSelectFormData>({
        mode: "onSubmit",
        defaultValues,
    });

    const submit = () => {
        handleSubmit((data: IngredientSelectFormData) => {
            if (isProduct(selectedBase)) {
                onEditConfirmed(
                    new ProductIngredient({
                        ...
                        serving: {
                            units:
                                selectedMeasuringType === ProductMeasuring.Units
                                    ? Number(data.measuringUnits)
                                    : FormatString.Weight(data.measuringUnits),
                ...
                <View style={{ margin: 2, backgroundColor: "white" }}>
                    <View style={{ padding: 5 }}>
                                        ...
                                        render={() => (
                                            ...
                                            >
                                                <Text
                                                    ...
                                                    variant="titleSmall"
                                                >
                                                    {selectedBase?.name ? selectedBase.name : t("product.search.noneSelected")}{" "}
                                                    {isPrepack(selectedBase) ? t("recipe.details.isPrepack") : ""}
                                                </Text>
                                            </Pressable>
                                        )}
                                    />
                                    ...
                                    <Controller
                                        ...
                                        render={({ field: { value, onChange } }) => (
                                            <TextInput
                                                ...
                                                label={t(
                                                    selectedMeasuringType === ProductMeasuring.Units
                                                        ? "product.measuring.units"
                                                        : "product.measuring.grams",
                                                )}
                        ...
                        {errors.ingredientBase && (
                            <Text
                                testID={collectionElementId(TestIds.IngredientSelect.Ingredient.RequiredError, index)}
                                style={styles.validationErrorLabel}
                            >
                                {t("validation.required.selectProduct")}
                            </Text>
                        )}
                        {errors.measuringUnits && (
                            <Text
                                testID={collectionElementId(TestIds.IngredientSelect.Ingredient.UnitsError, index)}
                                style={styles.validationErrorLabel}
                            >
